# Archived prompt — DOMAIN_ARCHITECT phase (02)

> Re-run with human feedback: "need to ensure card belong to this user_id".
...
ADDITIONAL FEEDBACK FROM HUMAN REVIEW:
- Ensure the card belongs to the given user_id: a well-formed card that is not owned by
  the user (or is owned by a different user) must be rejected with "CARD_NOT_OWNED" and
  no debit.
...
OUTPUT & ARCHIVAL:
- Save the final `.feature` file to `.features/withdraw/final/withdraw.feature`.
- Save this exact prompt to `.features/withdraw/prompts/02_domain_architect_prompt.md`.
